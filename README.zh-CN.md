...
AlphaSift 是为 AI Agent 设计的全市场候选发现与横向排序引擎。它按 YAML 策略筛选、评分、排序股票候选，并把运行结果保存下来，方便后续做 T+N 评估。
...
下面是 2026-04-12（周六，非交易时段）的一组真实无 LLM 记录，数据来自上一交易日 2026-04-10 收盘快照。示例只说明引擎输出形态，不构成任何推荐。
...
| `DAILY_SOURCE` | 否 | 日 K 数据源：`local`、`auto`、`tencent`、`sina`、`akshare`、`baostock` 或 `tushare`；有 Tushare token 时 `auto` 优先 `tushare` | `auto` |
...
- **候选级日 K 增强**：默认只对 L1 后 Top N 候选补充 MA、60 日涨幅、MACD/RSI、signal_score、20 日突破幅度、区间振幅、20 日量能比、实体强度、MA20 回踩距离和平台持续天数；`DAILY_SOURCE=auto` 且配置了 Tushare token 时会先试 `tushare`，否则优先走 `tencent`、`sina` 直连源，再降级到 `akshare`、`baostock`；启用 `DAILY_ENRICH_FULL_POOL=true` + `DAILY_SOURCE=local` 时，对快照硬筛后**全部**候选做日 K 硬筛（方案 C，详见 [docs/plans/2026-06-26-full-daily-k-hard-filter.md](docs/plans/2026-06-26-full-daily-k-hard-filter.md)）；低质量、拉取失败或 stale cache 行会进入最终风险扣分

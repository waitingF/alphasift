...
def _df_to_picks(df: pd.DataFrame) -> list[Pick]:
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        picks.append(Pick(
            ...
            zg_short=_safe_float(row.get("zg_short")),
            zg_long=_safe_float(row.get("zg_long")),
            kdj_k=_safe_float(row.get("kdj_k")),
            kdj_d=_safe_float(row.get("kdj_d")),
            kdj_j=_safe_float(row.get("kdj_j")),
            prev_kdj_j=_safe_float(row.get("prev_kdj_j")),
            boll_upper=_safe_float(row.get("boll_upper")),
            boll_lower=_safe_float(row.get("boll_lower")),
            daily_amplitude_pct=_safe_float(row.get("daily_amplitude_pct")),
            daily_change_pct=_safe_float(row.get("daily_change_pct")),
            zg_short_above_long=_safe_bool(row.get("zg_short_above_long")),
            close_above_zg_long=_safe_bool(row.get("close_above_zg_long")),
            brick_turn_up=_safe_bool(row.get("brick_turn_up")),
